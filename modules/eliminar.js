exports.eliminar = function(respuesta) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, function(err, db) {
        ...
        dbo.collection("pokemon").deleteOne({ "base.Speed": 100 });
        dbo.collection("pokemon").deleteOne({ $and: [{ id: { $gt: 350 } }, { id: { $lt: 380 } }] });
        ...
        respuesta.write('<!DOCTYPE html><html lang="es">' +
            ...
            '<a class="nav-link active" href="/eliminar">Eliminar</a>' +
            ...
            '<div class="row">' +
            '<div class="col-12">' +
            '<h1 class="mt-5">Eliminados correctamente</h1>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</body>' +
