exports.modificar = function(respuesta) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, function(err, db) {
        ...
        dbo.collection("pokemon").updateMany({ type: "Flying" }, { $set: { "base.Defense": 200 } });
        dbo.collection("pokemon").updateMany({ $and: [{ id: { $gt: 1 } }, { id: { $lt: 150 } }] }, { $set: { "Favorito": true } });
        ...
        respuesta.write('<!DOCTYPE html><html lang="es">' +
            ...
            '<a class="nav-link active" href="/modificar">Modificar</a>' +
            ...
            '<div class="row">' +
            '<div class="col-12">' +
            '<h1 class="mt-5">Modificaciones Realizadas</h1>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</body>' +
